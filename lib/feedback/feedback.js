...
function createFeedback({ transport, settings = {}, logger = console, logEvent = () => {} } = {}) {
  ...
  // Global (account) presence — 'available' | 'unavailable'. Best-effort like safePresence.
  const safeUserPresence = (state) => {
    try {
      Promise.resolve(transport.setUserPresence(state)).then(
        () => { emit('feedback-user-presence', { state }); logger.debug?.(`[feedback] user-presence ${state} -> ok`); },
        (e) => { emit('feedback-user-presence-failed', { state, error: e?.message }); logger.debug?.(`[feedback] user-presence ${state} -> ERR ${e?.message}`); },
      );
    } catch (e) {
      emit('feedback-user-presence-failed', { state, error: e?.message });
      logger.debug?.(`[feedback] setUserPresence(${state}) threw: ${e?.message}`);
    }
  };

  // One "I'm online and typing" beat. WhatsApp only delivers the "typing…" indicator while
  // the account is marked online, and that online state lapses mid-turn — so each beat
  // re-announces global `available` alongside refreshing the one-shot `composing` chat
  // state. Without the available re-announce the indicator flickers (shows, then drops
  // until the next turn). Sent at begin and every REFRESH_MS.
  //
  // `paused` at end stops chat typing; we deliberately NEVER send `unavailable`. User
  // presence is global (per connection, not per chat), so an ending/capped turn sending
  // `unavailable` would knock a *concurrent* chat's turn offline mid-typing. The account's
  // idle online-status between turns is therefore left as WuzAPI/whatsmeow manage it (it
  // announces `available` on connect); observe the live bot to confirm whether that means
  // online-while-working or persistently online.
  const sustainTyping = (chatJid) => {
    safeUserPresence('available');
    safePresence(chatJid, 'composing');
  };
  ...
  function begin(msg, { isReplay = false } = {}) {
    ...
    const st = { reactor, chatJid: msg.chatJid, chatType, typing: wantTyping, timer: null, capTimer: null, paused: false, agentReacted: false, subagents: 0, ended: false };
    ...
    if (wantTyping) {
      sustainTyping(msg.chatJid);
      st.timer = setInterval(() => sustainTyping(msg.chatJid), REFRESH_MS);
      st.timer.unref?.();
  ...
  // Called by the channels tool-dispatcher when a reply is delivered mid-turn. In the
  // channels model the turn keeps running after the reply (agent quiet-window / post-reply
  // work), so the typing loop would keep refreshing `composing` and the indicator would
  // re-appear AFTER the message — reading as "typing when it already answered". A sent
  // message means "done typing", so stop the loop now; end() still owns the reactor's
  // terminal resolution. Idempotent + best-effort (never throws into the dispatcher).
  function onReplyDelivered(sessionKey) {
    try {
      const st = turns.get(sessionKey);
      if (!st || st.ended || !st.typing || st.paused) return;
      stopTyping(st);
      emit('feedback-reply-delivered', { chatJid: st.chatJid });
    } catch (e) { logger.debug?.(`[feedback] onReplyDelivered: ${e?.message}`); }
  }
  ...
  return { begin, onEvent, markAgentReacted, onReplyDelivered };
}
