...
function createFeedback({ transport, settings = {}, logger = console } = {}) {
  ...
  function begin(msg, { isReplay = false } = {}) {
    ...
    const st = { reactor, chatJid: msg.chatJid, chatType, timer: null, capTimer: null, paused: false, agentReacted: false, ended: false };
    turns.set(sessionKey, st);

    if (ackFires(chatType)) reactor.setState('QUEUED'); // 👀 — the initial "I saw you" signal

    if (typingCfg.enabled) {
  ...
  function onEvent(sessionKey, kind, payload) {
    ...
    try {
      switch (kind) {
        ...
        case 'subagent-start':
          st.reactor.setWorkInFlight?.(true, 'subagent');
          st.reactor.setState('SUBAGENT');
          break;
        case 'subagent-done':
          st.reactor.setWorkInFlight?.(false, 'subagent');
          st.reactor.setState('THINKING');
          break;
        default:
  ...
  function end(sessionKey, { ok = false, delivered = false } = {}) {
    ...
    try {
      ...
      } else if (!st.agentReacted) {
        st.reactor.clear();                     // success + reply delivered → a clean text answer
      }
      // agentReacted: leave the agent's deliberate reaction untouched.
      st.reactor.stop();
