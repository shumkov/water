...
function createFeedback({ transport, settings = {}, logger = console, logEvent = () => {} } = {}) {
  ...
  // the account is marked online, and that state lapses — so each beat re-announces global
  // `available` BEFORE refreshing the one-shot `composing` chat state. Without the available
  // re-announce the indicator flickers (shows, then drops until the next turn). Sent at
  // begin and every REFRESH_MS. `paused` at end stops chat typing; we deliberately do NOT
  // send `unavailable` — user presence is global, so an ending turn must not knock another
  // chat's concurrent turn offline (the bot simply lapses to offline on its own when idle).
