...
function createTransportWatchdog({
  ...
  async function poll() {
    ...
    if (!standby && expectedWebhook && st) {
      ...
      // "Ours" = same webhook PATH (/hook/<pathToken>), regardless of host — so an
      // advertiseHost change (loopback → docker-bridge gateway) self-heals instead of looking
      // like a foreign consumer and dead-locking. A genuinely foreign consumer uses a different
      // path/token. Fall back to the host prefix when no path is supplied (legacy callers).
      const foreign = cur && (expectedWebhook.path
        ? !cur.endsWith(expectedWebhook.path)
        : !cur.startsWith(expectedWebhook.baseUrlPrefix || 'http://127.0.0.1'));
      if (!cur || (!foreign && cur !== expectedWebhook.url)) {
        ...
        await escalate('INFO', `wuzapi webhook points at a different endpoint (${cur}) — expected path ${expectedWebhook.path || expectedWebhook.url}`);
      }
    }
  }
