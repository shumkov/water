...
function createSlaWatchdog({
  ...
  async function tick() {
    ...
    for (const row of stuck.all()) {
      ...
      // Elapsed is measured from when WE received it (received_at), not the WhatsApp
      // event time — an offline-replayed old message must not look instantly overdue.
      if (t - (row.received_at ?? row.ts) < holdAfterMs(row.chat_jid)) continue;
