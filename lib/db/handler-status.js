...
function createHandlerStatus(db) {
  ...
  // Explicit replay-pending rows survive the legacy replay window because shutdown
  // has already identified them as interrupted work.
    ...
    SELECT * FROM messages
     WHERE direction='in' AND is_from_me=0
       AND (
         ...
         OR (
           received_at >= @cutoff
           AND (handler_status IS NULL OR handler_status='dispatched')
         )
       )
  ...
  const markOneReplayPending = db.prepare(
    "UPDATE messages SET handler_status='replay-pending' WHERE id=?",
  );
  const resetOneForReplay = db.prepare(
    'UPDATE messages SET handler_status=NULL, error=NULL WHERE id=?',
  );
  ...
  return {
    ...
    markReplayPending: (id) => markOneReplayPending.run(id).changes,
    resetForReplay: (id) => resetOneForReplay.run(id).changes,
    TERMINAL,
  };
}
