...
function createJidMap(db) {
  ...
  // Record a (pn, lid) pair. Either side may be absent; a pair with only one known
  // side is not stored (nothing to map yet). The pair shape is validated
  // (pn is @s.whatsapp.net, lid is @lid) so no caller — seed() or a future one — can
  // store a swapped or garbage pair that would corrupt allowlist resolution.
  //
  // Trust boundary: on the message path the pair comes from whatsmeow's attested
  // Info.Sender/SenderAlt (bound to the sender's identity key in the WhatsApp
  // protocol), so a sender cannot forge a mapping to someone else's phone number.
  // We rely on that attestation; we do not accept sender-claimed identity from text.
  function observe({ pn, lid, pushName, ts }) {
    ...
    if (suffixKind(p) !== 'pn' || suffixKind(l) !== 'lid') return; // reject malformed pairs
