...
function createOutbound(db, { botJid = 'me', now = Date.now, sentCacheTtlMs = 24 * 3600_000 } = {}) {
  const insert = db.prepare(`
    INSERT INTO messages
      (chat_jid, msg_id, sender_jid, text, direction, source, account, is_from_me,
       turn_id, quote_msg_id, status, ts, received_at)
    VALUES
      (@chatJid, @msgId, @botJid, @text, 'out', @source, @account, 1,
       @turnId, @sourceMsgId, 'pending', @ts, @ts)
  `);
  ...
  const selAllPendingOut = db.prepare(
    `SELECT id, chat_jid, msg_id FROM messages WHERE direction='out' AND status='pending'`,
  );
  ...
  // Boot sweep: at boot EVERY pending outbound is from a prior life (this process has
  // reserved nothing yet), so mark ALL of them — no age filter. A crash seconds after
  // reserve() must still land in failed('crashed-mid-send'), not leak past the boot
  // sweep and get mislabeled 'ambiguous-send' 60s later. Distinct error so replay/dedup
  // can tell it apart from the runtime ambiguous case.
  function sweepCrashed() {
    const rows = selAllPendingOut.all();
