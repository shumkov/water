...
// Extension → MIME. Images MUST resolve to an image/* type: WuzAPI's /chat/send/image
// rejects a non-image data-URL MIME with HTTP 400, so a wrong MIME makes image sends fail
// while documents (any MIME) succeed. Unknown types fall back to octet-stream (document path).
const MIME_BY_EXT = {
...
function buildAllowedRoots({ sessionKey, sessionCwd = null, extraRoots = [], base = DEFAULT_ATTACHMENT_BASE }) {
  // RAW sessionKey — must byte-match cli-process's staging dir (path.join(base,
  // String(sessionKey))). WhatsApp JIDs contain '@'/':'; sanitizing here diverged the
  // allowlist from the real dir so every reply(files) was rejected.
...
function createChannelsToolDispatcher({
  ...
  logger = console,
  now = Date.now,
  feedback = null,   // per-turn feedback controller — flagged when the agent reacts on the source msg
} = {}) {
  ...
  async function sendOne({ chatJid, text, quote, turnId, sourceMsgId }) {
    const { rowId, msgId } = outbound.reserve({
      chatJid,
      text,
      account,
      source: 'bot-reply',
  ...
  return async function channelsToolDispatcher(call) {
    ...
    if (toolName === 'react') {
      ...
      // participantJid names the SOURCE message's author, so it's a correct reaction target
      // only when reacting to that same message. For any other target we don't know the
      // author — pass none rather than mislabel it.
      const onSourceMsg = sourceMsgId != null && String(messageId) === String(sourceMsgId);
      // If the agent reacts on THIS turn's source message, tell the feedback controller so its
      // end-of-turn ack-clear won't stomp the agent's deliberate reaction (FEEDBACK_SPEC §4.3).
      if (onSourceMsg) feedback?.markAgentReacted?.(sessionKey);
      try {
        await transport.react({ chatJid, msgId: messageId, emoji: text || null, participantJid: onSourceMsg ? participantJid : undefined });
        return { ok: true };
    ...
    if (Array.isArray(files) && files.length && failed.length === 0) {
      ...
      for (const f of files.slice(0, MAX_FILES_PER_REPLY)) {
        ...
        try {
          // Send with the file's REAL content type. WuzAPI's /chat/send/image validates the
          // data-URL MIME and rejects a non-image type with HTTP 400, so a generic
          // application/octet-stream makes every image fail while documents (which accept any
          // MIME) still go through — i.e. images silently never send. Derive the MIME from the
          // extension and use it in BOTH the data-URL prefix and the MimeType field.
          const mime = mimeForFile(v.real);
          const kind = mime.startsWith('image/') ? 'image' : 'document';
          const data = `data:${mime};base64,${fs.readFileSync(v.real).toString('base64')}`;
          const { rowId, msgId } = outbound.reserve({
            chatJid,
            text: `[file:${path.basename(v.real)}]`,
            account,
            source: 'bot-reply',
            turnId,
            sourceMsgId,
          });
          const res = await transport.sendMedia({ chatJid, kind, data, mimeType: mime, fileName: path.basename(v.real), id: msgId });
          outbound.markSent(rowId, res?.ts);
    ...
    // A reply landed — stop the "typing…" loop so it doesn't re-appear after the message
    // (the turn keeps running past the reply in the channels model). Best-effort.
    if (sent.length > 0) feedback?.onReplyDelivered?.(sessionKey);
    if (failed.length > 0) {
