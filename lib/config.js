...
function validateConfig(cfg) {
  ...
  for (const [name, acc] of Object.entries(cfg.accounts)) {
    ...
    if (acc.webhook.requireHmac === false) {
      req(!acc.wuzapi.hmacKey,
        `${at}.wuzapi.hmacKey must be empty when webhook.requireHmac is false`);
    }
    ...
    if (acc.dmRestrictedReply !== undefined) {
      req(typeof acc.dmRestrictedReply === 'string', `${at}.dmRestrictedReply must be a string`);
    }
  }
