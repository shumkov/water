...
function validateConfig(cfg) {
  ...
  for (const [name, acc] of Object.entries(cfg.accounts)) {
    ...
    if (acc.ackReaction !== undefined) {
      for (const k of Object.keys(acc.ackReaction)) {
        req(ACK_MODES.has(acc.ackReaction[k]), `${at}.ackReaction.${k} must be never|mentions|always`);
      }
    }
