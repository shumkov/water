...
/**
 * Unix socket IPC server for the Water daemon.
 *
 * One socket per account in Water's owner-only IPC runtime directory. Clients
 ...
 */
function writeSecret(botName, options) {
  const secret = crypto.randomBytes(32).toString('base64url');
  const p = secretPathFor(botName, options);
