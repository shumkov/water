...
function runMigrations(db, dir = MIGRATIONS_DIR) {
  ...
  for (const { num, path: file } of migrations) {
    ...
    // Migrations that create WAL/journal pragmas can't run inside a transaction in
    // SQLite; run those statements (PRAGMA journal_mode = WAL etc.) before the txn
    // by exec-ing the file's pragmas separately is unnecessary here because
    // better-sqlite3 executes journal_mode outside the txn automatically. If a
    // migration ever fails mid-way the txn rolls back and boot aborts loudly.
    applied();
  }
