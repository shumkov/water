...
const PROCESS_LOSS_RESET_FAILED_MESSAGE =
  '🔄 That Claude session stopped before the turn finished, and I couldn\'t safely reset its saved state. Please try /reset before resending; if that fails, contact the operator.';
...
function createDispatcher({
  ...
  async function dispatch(sessionKey, msg, row, { isReplay = false, shutdownToken = null } = {}) {
    ...
    try {
      ...
      if (shutdownToken) inFlightTokens.set(sessionKey, shutdownToken);
      ...
      let userMessage = info?.userMessage;
      ...
      // A lost contained process cannot service a resume. Clear its persisted
      // session before any user guidance so the requested resend starts fresh.
      if (err.code === 'SESSION_PROCESS_LOST' && typeof sessions.clearSession === 'function') {
        try {
          sessions.clearSession(sessionKey);
          logEvent('session-reset-after-process-loss', { sessionKey, msgId: msg.msgId });
        } catch (resetErr) {
          userMessage = PROCESS_LOSS_RESET_FAILED_MESSAGE;
          logEvent('session-reset-after-process-loss-failed', { sessionKey, msgId: msg.msgId });
          logger.error?.('session-reset-after-process-loss', resetErr?.message);
        }
      }
      ...
      if (!isReplay && userMessage && errorReply) {
        try { await errorReply(msg, userMessage); } catch { /* best effort */ }
      }
      throw err;
    } finally {
      // Resolve feedback (best-effort, must not throw), THEN always release the lock.
      inFlightSenders.delete(sessionKey);
      if (inFlightTokens.get(sessionKey) === shutdownToken) inFlightTokens.delete(sessionKey);
  ...
  return {
    dispatch,
    spawnContextFor,
    inFlightSender: (sk) => inFlightSenders.get(sk),
    inFlightCount: () => inFlightSenders.size,
    inFlightToken: (sk) => inFlightTokens.get(sk) || null,
  };
}
