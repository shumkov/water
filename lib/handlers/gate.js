...
//   restrictedDmEnabled: bool  (config `dmRestrictedReply` is a non-empty string)
//   hasRecentRestrictedReply(chatJid, senderJid) -> bool  (per-sender rate cap)
function createGate(deps) {
  const {
    ...
    restrictedDmEnabled = false,
    hasRecentRestrictedReply = () => false,
  } = deps;
  ...
  function decide(msg) {
    ...
    if (!chat) {
      ...
      // aren't monitored" note (no turn), capped at one per sender per rolling window so
      // an auto-responder loop or a DM flood can't run unbounded. Groups stay a silent
