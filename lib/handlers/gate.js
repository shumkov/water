...
function createGate(deps) {
  ...
  function isMentioned(msg, chat) {
    ...
    // name-trigger regexes (mentionPatterns), case-insensitive
    for (const pat of chat.mentionPatterns ?? []) {
      try { if (new RegExp(pat, 'i').test(msg.text ?? '')) return true; } catch { /* validated at load */ }
    }
    return false;
  }
  ...
  function decide(msg) {
    ...
    // 4. admin / verdict commands (adminJids only)
    ...
    if (ADMIN_CMD.test(msg.text ?? '') && isAdmin(msg.sender.jid)) {
