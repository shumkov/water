...
function parseAnswer(text, q) {
  ...
  // 3. prefix label match — the OPTION label starts with the reply (e.g. "sm" → "Small").
  //    NOT a substring/contains match: a free-text reply that merely CONTAINS a short label
  //    ("not sure" ⊃ "no", "cashier's cheque" ⊃ "cash") must fall through to the free-text
  //    `other` branch below, not be silently coerced onto that option.
  const partial = options.filter((o) => o && o.toLowerCase().startsWith(lc));
