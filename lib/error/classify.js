...
const CODES = {
  ...
  // The isolated process tree exited, so its persisted session cannot be
  // resumed safely. The dispatcher clears that session before surfacing this
  // guidance, and the user's resend starts a fresh process.
  SESSION_PROCESS_LOST: {
    kind: 'sessionProcessLost',
    userMessage: '🔄 That Claude session stopped before the turn finished. Please resend your message and I\'ll start a fresh session.',
    isTransient: false,
    autoRecover: null,
  },
  ...
  // null) — a billing/infra outage on our end isn't their problem.
  AUTH_DISABLED: {
    kind: 'authDisabled',
    userMessage: null,
    isTransient: false,
