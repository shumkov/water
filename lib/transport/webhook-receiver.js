...
//
// HTTP on `<bindHost>:<port>/hook/<pathToken>` (bindHost defaults to loopback; a
// Docker-networked deployment binds a wider interface — see water.js). Per delivery: read body (8 MB sanity cap —
...
function createReceiver({ port, host = '127.0.0.1', pathToken, hmacKey, skipHmac = false, handlers, healthPayload = () => ({}), logger = console, emit = () => {}, bodyCap = BODY_CAP }) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      // HMAC gate. skipHmac (explicitly opted in via webhook.requireHmac:false AND no hmacKey)
      // accepts unsigned webhooks; the boundary is then the bind host + host firewall.
      if (!skipHmac && !verify(raw, req.headers['x-hmac-signature'], hmacKey)) {
  ...
  return {
    listen() {
      // Reject on a bind error (e.g. EADDRNOTAVAIL when binding a specific interface that
      // isn't up yet at boot) instead of leaving an unhandled 'error' event to crash the
      // process — the caller exits non-zero and systemd (Restart=on-failure) retries cleanly.
      return new Promise((resolve, reject) => {
        const onError = (err) => reject(err);
        server.once('error', onError);
        server.listen(port, host, () => { server.removeListener('error', onError); resolve(server.address()); });
      });
    },
    close() {
