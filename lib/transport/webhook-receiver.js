...
function createReceiver({ port, host = '127.0.0.1', pathToken, hmacKey, skipHmac = false, handlers, healthPayload = () => ({}), logger = console, emit = () => {}, bodyCap = BODY_CAP }) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      // HMAC gate. skipHmac (loopback-trust — explicitly opted in via webhook.requireHmac:false
      // AND no hmacKey) accepts unsigned webhooks; the 127.0.0.1 bind is the boundary.
